...
inline constexpr bool operator==(TKey a, TKey b) noexcept
{
    return bool((a.code | ((int32_t)a.mods << 16)) == (b.code | ((int32_t)b.mods << 16)));
}

inline constexpr bool operator!=(TKey a, TKey b) noexcept { return bool(!(a == b)); }

inline ipstream& operator>>(ipstream& is, TKey& p) { return is >> p.code >> p.mods; }
inline ipstream& operator>>(ipstream& is, TKey*& p) { return is >> p->code >> p->mods; }

inline opstream& operator<<(opstream& os, TKey& p) { return os << p.code << p.mods; }
inline opstream& operator<<(opstream& os, TKey* p) { return os << p->code << p->mods; }
