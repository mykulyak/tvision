...
static BOOL(WINAPI* pSetConsoleActiveScreenBuffer)(HANDLE) = (BOOL(WINAPI*)(HANDLE))getModuleProc("KERNEL32", "SetConsoleActiveScreenBuffer");
static BOOL(WINAPI* pOpenClipboard)(HWND) = (BOOL(WINAPI*)(HWND))getModuleProc("USER32", "OpenClipboard");
static BOOL(WINAPI* pCloseClipboard)() = (BOOL(WINAPI*)())getModuleProc("USER32", "CloseClipboard");
static HANDLE(WINAPI* pSetClipboardData)(UINT, HANDLE) = (HANDLE(WINAPI*)(UINT, HANDLE))getModuleProc("USER32", "SetClipboardData");
static HANDLE(WINAPI* pGetClipboardData)(UINT) = (HANDLE(WINAPI*)(UINT))getModuleProc("USER32", "GetClipboardData");
static BOOL(WINAPI* pEmptyClipboard)() = (BOOL(WINAPI*)())getModuleProc("USER32", "EmptyClipboard");
    ...
    do {
        ...
        if (pendingEvent && irBuffer.EventType != MOUSE_EVENT) {
            ...
            if (irBuffer.EventType == KEY_EVENT && irBuffer.Event.KeyEvent.bKeyDown) {
                ...
                if (event.keyDown.keyCode == 0x2A00 || event.keyDown.keyCode == 0x1D00 || event.keyDown.keyCode == 0x3600 || event.keyDown.keyCode == 0x3800 || event.keyDown.keyCode == 0x3A00)
                    // Discard standalone Shift, Ctrl, Alt, Caps Lock keys.
                    event.keyDown.keyCode = kbNoKey;
                else if ((event.keyDown.controlKeyState & kbCtrlShift) && (event.keyDown.controlKeyState & kbAltShift)) // Ctrl+Alt is AltGr.
                {
                    ...
                    else if (!(event.keyDown.controlKeyState & (kbShift | kbCtrlShift | kbAltShift)) && NormalCvt[index] != 0)
...
BOOL THardwareInfo::setClipboardText(TStringView text)
{
    ...
    if (pOpenClipboard && pOpenClipboard(0)) {
        HGLOBAL hData = NULL;
        char* pData;
        if (pEmptyClipboard() && (result = text.empty()) == 0 && (hData = GlobalAlloc(GMEM_MOVEABLE, text.size() + 1)) != 0 && (pData = (char*)GlobalLock(hData)) != 0) {
